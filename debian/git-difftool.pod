...
B<git difftool> [--tool=<tool>] [--ref=<ref>] [--no-prompt] [<file>]...

=head1 DESCRIPTION

B<git-difftool> is a git command that allows you to compare and edit files against arbitrary revisions using the standard git merge tools.
...
-r or --ref=<ref>
    Specifies a commit-ish reference to compare against. B<git-difftool> compares against the index by default.
...
This manual page was written by Iulian Udrea <iulian@ubuntu.com>.
